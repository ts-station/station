...
/**
 *分隔字符串中的空格或逗号
 *@param string $str 带有空格或逗号的字符串
 *@return array $strarr返回数组
 *@author 纺友
 */
 function split_string($str){
	...
	$str=trim($str);
	if(strpos($str,',')){
		$strtemparr1=explode(',',$str);	
		foreach($strtemparr1 as $val1){
			if(strpos(trim($val1),' ')){
				$strtemparr2[]=explode(' ',trim($val1));
			}else{
				$strtemparr3[]=trim($val1);
			}
		}
		foreach($strtemparr2 as $val2){
			foreach($val2 as $val3){
				$strtemparr4[]=$val3;
			}
		}
		$strarr=array_merge($strtemparr3,$strtemparr4);
		return $strarr;
	}
	if(strpos(trim($str),' ')){
